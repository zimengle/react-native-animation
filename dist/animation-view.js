...
import { Dimensions, findNodeHandle, UIManager, requireNativeComponent, Platform,Alert } from 'react-native';
...
export let AnimationView = class AnimationView extends React.Component {
    ...
    start() {
        ...
        console.info(Platform.OS,this.data);
        let params = Platform.OS === 'android' ? JSON.stringify(this.data || []) : this.data;
    ...
    render() {
        let opacity = 1;
        if(this.props.style && typeof this.props.style.opacity != 'undefined'){
            opacity = this.props.style.opacity;
        }

        return (<TBNAnimationView ref={this._assignRoot} >
                {this.props.children}
            </TBNAnimationView>);
    }
